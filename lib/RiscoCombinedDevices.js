...
class RiscoCPCombDevices {
    ...
    async getCurrentState(callback) {
        try {
            this.log.debug('%s "%s" MotionDetected: %s', this.sPrefix, this.name, ((this.InDeviceInstance.Open) ? this.InActiveStr : this.InInactiveStr));
            callback(null, (this.InDeviceInstance.Open) ? this.InActiveValue : this.InInactiveValue);
            return;
        } catch (err) {
            this.log.error('Error on RiscoCPCombDevices/getCurrentState:\n%s', err);
    ...
    try {
            ...
            this.log.error('Error on RiscoCPCombDevices/getTargetState:\n%s', err);
    ...
    async getCurrentExcludeState(callback) {
        try {
            ...
            this.log.error('Error on RiscoCPCombDevices/getCurrentExcludeState:\n%s', err);
    ...
    async setCurrentExcludeState(state, callback) {
        try {
            ...
            this.log.error('Error on RRiscoCPCombDevices/setCurrentExcludeState:\n%s', err);
            this.ExcludeService.updateCharacteristic(this.Characteristic.On, this.InDeviceInstance.Bypass);
            callback(err);
            return;
        }
